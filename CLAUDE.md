...
- Voice import review step (owner ask 2026-09-24): Stop → the transcript (server, else live preview) lands in an editable field → BUILD WORKOUT parses exactly that text (`VoiceTranscriptEditing`: 20,000-char cap mirrored, whitespace-only changes are not edits; re-record confirms when edited); no pause — the recorder has none.
